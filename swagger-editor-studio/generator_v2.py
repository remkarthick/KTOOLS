...
def default_openapi_spec():
    return {
        ...
        "x-studio": {
            "relationships": {
                "paths": {
                    "/users": {
                        "get": {
                            "requestSchemaRelation": {"type": "single", "items": []},
                            "requestExampleRelation": {"type": "single", "items": []},
                            "responseSchemaRelations": {
                                "200": {
                                    "type": "oneOf",
                                    "items": [
                                        {"kind": "ref", "value": "#/components/schemas/UserList"},
                                        {"kind": "inline", "value": {"type": "array", "items": {"$ref": "#/components/schemas/User"}}}
                                    ]
                                }
                            },
                            "responseExampleRelations": {
                                "200": {
                                    "type": "anyOf",
                                    "items": [
                                        {"name": "basic", "summary": "Basic response", "value": [{"id": 1, "name": "John", "email": "john@example.com"}]},
                                        {"name": "empty", "summary": "Empty response", "value": []}
                                    ]
                                }
                            }
                        },
                        "post": {
                            "requestSchemaRelation": {
                                "type": "allOf",
                                "items": [
                                    {"kind": "inline", "value": {"type": "object", "properties": {"name": {"type": "string"}}}},
                                    {"kind": "inline", "value": {"type": "object", "properties": {"email": {"type": "string"}}}}
                                ]
                            },
                            "requestExampleRelation": {
                                "type": "anyOf",
                                "items": [
                                    {"name": "createUser", "summary": "Create user payload", "value": {"name": "John", "email": "john@example.com"}},
                                    {"name": "minimal", "summary": "Minimal payload", "value": {"name": "Jane", "email": "jane@example.com"}}
                                ]
                            },
                            "responseSchemaRelations": {
                                "201": {
                            ...
                            "responseExampleRelations": {
                                "201": {
                                    "type": "single",
                                    "items": [
                                        {"name": "created", "summary": "Created user", "value": {"id": 2, "name": "John", "email": "john@example.com"}}
                                    ]
                                }
                            }
                        }
        ...
        "info": {
            "title": "Swagger Studio v11.1",
            "description": "OpenAPI editor with explicit relationships and schema creation from examples.",
            "version": "1.0.0"
        },
...
<head>
    ...
    <title>Swagger Studio v11.1</title>
...
<body class="studio-body">
    <nav class="navbar navbar-expand-lg navbar-dark bg-dark border-bottom shadow-sm">
        <div class="container-fluid">
            <a class="navbar-brand fw-bold" href="#">Swagger Studio v11.1</a>
                ...
                <div class="card h-100 border-0 shadow-sm">
                    <div class="card-header d-flex justify-content-between align-items-center bg-info-subtle">
                        <div>
                            ...
                            <div class="small text-muted">Multiple schemas, multiple examples, create schema from example</div>
                        </div>
                    </div>
                    <div class="card-body panel-scroll">
    ...
    <div class="modal fade" id="schemaFromExampleModal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
            <div class="modal-content">
                <div class="modal-header sticky-top bg-body">
                    <div>
                        <h5 class="modal-title mb-0" id="schemaFromExampleModalTitle">Create Schema From Example</h5>
                        <div class="small text-muted" id="schemaFromExampleModalSubtitle">Generate reusable schema and attach it to relationship</div>
                    </div>
                    <button type="button" class="btn-close" onclick="closeSchemaFromExampleModal()"></button>
                </div>
                <div class="modal-body" id="schemaFromExampleModalBody"></div>
                <div class="modal-footer sticky-bottom bg-body">
                    <button type="button" class="btn btn-secondary" onclick="closeSchemaFromExampleModal()">Cancel</button>
                    <button type="button" class="btn btn-primary" onclick="saveSchemaFromExampleModal()">Create Schema</button>
                </div>
            </div>
        </div>
    </div>
...
let schemaFromExampleModal = null;
let currentOperationContext = null;
let relationshipItemContext = null;
let schemaFromExampleContext = null;
...
function openSchemaFromExampleModal(title, subtitle, html, context) {
    schemaFromExampleContext = context;
    document.getElementById("schemaFromExampleModalTitle").textContent = title;
    document.getElementById("schemaFromExampleModalSubtitle").textContent = subtitle || "";
    document.getElementById("schemaFromExampleModalBody").innerHTML = html;
    schemaFromExampleModal.show();
}

function closeSchemaFromExampleModal() {
    schemaFromExampleContext = null;
    schemaFromExampleModal.hide();
}
        ...
        <div class="accordion">
            ${accordionItem("info-general", "General", `
                <div class="row g-3">
                    ...
                    <div class="col-12 sticky-modal-actions">
...
function openOperationEditor(pathKey, method) {
    ...
    openModalEditor(`${method.toUpperCase()} ${pathKey}`, "Manage multiple schemas, multiple examples and relationships.", `
        <div class="accordion">
            ...
            ${accordionItem("op-request-schema-rel", "Request Schema Relationship", renderSchemaRelationshipEditor(pathKey, method, rel.requestSchemaRelation, "requestSchema"))}
            ${accordionItem("op-request-example-rel", "Request Example Relationship", renderExampleRelationshipEditor(pathKey, method, rel.requestExampleRelation, "requestExample"))}
            ...
            ${accordionItem("op-openapi-note", "Design Notes", `
                <ul class="mb-0">
                    <li>Use multiple schema items when endpoint payload can vary.</li>
                    <li>Use <code>oneOf</code>, <code>anyOf</code>, or <code>allOf</code> for schema composition.</li>
                    <li>Create reusable schemas directly from examples for better reuse.</li>
                    <li>Attach newly generated component schemas back into request/response relationships.</li>
                </ul>
            `)}
        </div>
    `);
...
function renderSchemaRelationshipEditor(pathKey, method, relation, targetType, code = null) {
    ...
    return `
        ...
        <div class="d-flex gap-2 mb-3 flex-wrap">
            ...
            <button class="btn btn-sm btn-outline-secondary" onclick='openSchemaRelationItemEditor(${JSON.stringify(targetType)}, ${code ? JSON.stringify(code) : "null"}, null)'>Add Schema Item</button>
        </div>
        ${listHtml}
    `;
}
...
function renderExampleRelationshipEditor(pathKey, method, relation, targetType, code = null) {
    relation = relation || relationTemplate();
    const typeId = code ? `${targetType}-type-${code}` : `${targetType}-type`;
    const selectId = code ? `${targetType}-example-select-${code}` : `${targetType}-example-select`;
    ...
    const selector = (relation.items || []).length ? `
        <div class="mb-3">
            <label class="form-label">Select Example</label>
            <select id="${selectId}" class="form-select">
                <option value="">Choose example</option>
                ${(relation.items || []).map((item, idx) => `<option value="${idx}">${escapeHtml(item.name || `Example ${idx + 1}`)}</option>`).join("")}
            </select>
        </div>
    ` : "";
    ...
    return `
        ...
        <div class="d-flex gap-2 mb-3 flex-wrap">
            ...
            <button class="btn btn-sm btn-outline-success" onclick='openCreateSchemaFromExampleFlow(${JSON.stringify(targetType)}, ${code ? JSON.stringify(code) : "null"})'>Create Schema From Example</button>
        </div>
        ${listHtml}
    `;
}
...
function renderResponseRelationshipCard(pathKey, method, code) {
    ...
    return `
        <div class="card mb-3">
            ...
            <div class="card-body">
                <div class="accordion">
                    ${accordionItem(`res-schema-${slugify(code)}`, `Response Schema Relationship (${schemaRel.type || "single"})`, renderSchemaRelationshipEditor(pathKey, method, schemaRel, "responseSchema", code))}
                    ${accordionItem(`res-example-${slugify(code)}`, `Response Example Relationship (${exampleRel.type || "single"})`, renderExampleRelationshipEditor(pathKey, method, exampleRel, "responseExample", code))}
                </div>
            </div>
        </div>
    `;
...
function saveRelationType(targetType, code = null) {
    ...
    syncAllViews();
    openOperationEditor(currentOperationContext.pathKey, currentOperationContext.method);
}
...
function openSchemaRelationItemEditor(targetType, code, index) {
    ...
    openRelationshipItemModal(
        ...
        "Add or edit an inline schema or $ref schema item.",
        `
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <select id="rel-schema-kind" class="form-select" onchange="toggleSchemaItemEditorMode()">
                    <option value="inline" ${item.kind === "inline" ? "selected" : ""}>inline</option>
                    <option value="ref" ${item.kind === "ref" ? "selected" : ""}>ref</option>
                </select>
            </div>
            <div class="col-12 ${item.kind === "ref" ? "" : "hidden"}" id="rel-schema-ref-wrap">
                <label class="form-label">Schema Reference</label>
            ...
            <div class="col-12 ${item.kind === "inline" ? "" : "hidden"}" id="rel-schema-inline-wrap">
...
function toggleSchemaItemEditorMode() {
    const kind = document.getElementById("rel-schema-kind").value;
    document.getElementById("rel-schema-ref-wrap").classList.toggle("hidden", kind !== "ref");
    document.getElementById("rel-schema-inline-wrap").classList.toggle("hidden", kind !== "inline");
}
...
function openCreateSchemaFromExampleFlow(targetType, code = null) {
    const relation = getRelationObject(targetType, code);
    if (!relation || !(relation.items || []).length) {
        return alert("No examples available in this relationship.");
    }

    const options = relation.items.map((item, idx) =>
        `<option value="${idx}">${escapeHtml(item.name || `Example ${idx + 1}`)}</option>`
    ).join("");

    openSchemaFromExampleModal(
        "Create Schema From Example",
        "Generate a reusable component schema from a selected example and optionally attach it.",
        `
        <div class="row g-3">
            <div class="col-md-6">
                <label class="form-label">Select Example</label>
                <select id="schema-from-example-index" class="form-select">
                    <option value="">Choose example</option>
                    ${options}
                </select>
            </div>
            <div class="col-md-6">
                <label class="form-label">New Schema Name</label>
                <input id="schema-from-example-name" class="form-control" placeholder="GeneratedSchema">
            </div>
            <div class="col-md-6">
                <label class="form-label">Attach Generated Schema To</label>
                <select id="schema-from-example-attach" class="form-select">
            ...
            <div class="col-md-6">
                ...
                <select id="schema-from-example-kind" class="form-select">
                    <option value="ref" selected>Reusable $ref item</option>
                    <option value="inline">Inline schema item</option>
                </select>
            </div>
            <div class="col-12">
                <div class="form-text">
                    This creates a component schema from the chosen example. If attach is enabled, it will also be added into the corresponding schema relationship list so multiple schemas are supported.
                </div>
            </div>
        </div>
        `,
...
function saveSchemaFromExampleModal() {
    if (!schemaFromExampleContext) return;

    const { targetType, code } = schemaFromExampleContext;
    const exampleRelation = getRelationObject(targetType, code);
    if (!exampleRelation) return;

    const idx = document.getElementById("schema-from-example-index").value;
    const schemaName = document.getElementById("schema-from-example-name").value.trim();
    const attach = document.getElementById("schema-from-example-attach").value === "yes";
    const attachKind = document.getElementById("schema-from-example-kind").value;

    if (idx === "") return alert("Please select an example.");
    if (!schemaName) return alert("Please enter a schema name.");

    ensureComponents();
    if (currentSpec.components.schemas[schemaName]) {
        return alert("Schema name already exists.");
    }

    const exampleItem = exampleRelation.items[Number(idx)];
    if (!exampleItem) return alert("Selected example not found.");

    const generatedSchema = inferSchemaFromExample(exampleItem.value);
    currentSpec.components.schemas[schemaName] = generatedSchema;

    if (attach) {
        const schemaTargetType = targetType === "requestExample" ? "requestSchema" : "responseSchema";
        const schemaRelation = getRelationObject(schemaTargetType, code);
        if (!schemaRelation) return alert("Target schema relationship not found.");

        if (attachKind === "ref") {
            schemaRelation.items.push({
                kind: "ref",
...
window.onload = async function () {
    ...
    schemaFromExampleModal = new bootstrap.Modal(document.getElementById("schemaFromExampleModal"));
...
README = r'''# Swagger Studio v11.1

Features:
- explicit relationship model between endpoints, schemas, and examples
- supports multiple schemas per request/response relationship
- supports multiple examples per request/response relationship
- create schema from selected example
- generated schema saved into components.schemas
- optionally attach generated schema back to request/response schema relationship
- relationship types:
  - single
  - oneOf
  - anyOf
  - allOf
- x-studio relationship metadata preserved
- valid OpenAPI 3.x output generated
- Bootstrap 5 UI
- raw YAML/JSON panel
- live preview
